library dropdown_search_edited;
...
class DropdownSearchEdited<T> extends StatefulWidget {
  ...
  DropdownSearchEdited.multiSelection({
    Key? key,
    this.autoValidateMode = AutovalidateMode.disabled,
    this.items = const [],
    this.asyncItems,
  ...
  @override
  DropdownSearchEditedState<T> createState() => DropdownSearchEditedState<T>();
}

class DropdownSearchEditedState<T> extends State<DropdownSearchEdited<T>> {
  ...
  @override
  void didUpdateWidget(DropdownSearchEdited<T> oldWidget) {
  ...
  Future _openMenu() {
    ...
    var overlay = Overlay.of(context).context.findRenderObject() as RenderBox; //removed !
